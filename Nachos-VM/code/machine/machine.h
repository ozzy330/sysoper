...
public:
  ...
  void DumpState(); // print the user CPU and memory DumpState

  // Devuelve si existe una pagina en Swap/Memoria, y la guarda en entry
  bool checkOnMem(int vpn, TranslationEntry *entry);
  bool checkOnSwap(int vpn, TranslationEntry *entry);

  // Implementa un algoritmo de remplazo de páginas
  void secondChanceTLB(TranslationEntry *entry);
