...
void Machine::OneInstruction(Instruction *instr) {
  ...
  case OP_ADDU:
    registers[(int)instr->rd] =
        registers[(int)instr->rs] + registers[(int)instr->rt];
    break;
  ...
  case OP_ANDI:
    registers[(int)instr->rt] =
        registers[(int)instr->rs] & (instr->extra & 0xffff);
    break;
  ...
  case OP_OR:
    registers[(int)instr->rd] =
        registers[(int)instr->rs] | registers[(int)instr->rs];
    break;
  ...
  case OP_SRAV:
    registers[(int)instr->rd] =
        registers[(int)instr->rt] >> (registers[(int)instr->rs] & 0x1f);
    break;
  ...
  case OP_SWL:
    ...
    case 1:
      value =
          (value & 0xff000000) | ((registers[(int)instr->rt] >> 8) & 0xffffff);
      break;
    case 2:
      value =
          (value & 0xffff0000) | ((registers[(int)instr->rt] >> 16) & 0xffff);
      break;
    case 3:
  ...
  // INFO: Ejecuta un llamado al sistema
  case OP_SYSCALL:
